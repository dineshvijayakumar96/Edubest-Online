...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO uiux_brochure_form_submissions (first_name, last_name, email, phone, experience, country, inquiry_for) VALUES (:first_name, :last_name, :email, :phone, :experience, :country, :inquiry_for)";
        ...
        try {
            ...
            $mailer->isHTML(true);
            $mailer->Subject = 'UIUX Designer Brochure Form Submission';

            $emailBody = "
                <html>
                <head>
                    <title>UIUX Designer Brochure Form Submission</title>
                </head>
                <body>
                    <h1>UIUX Designer Brochure Form Submission</h1>
            ...
            $brochureFile = 'uiux-brochure.pdf'; // Replace with the actual path to your brochure file
            if (file_exists($brochureFile)) {
                header('Content-Description: File Transfer');
                header('Content-Type: application/octet-stream');
                header('Content-Disposition: attachment; filename="uiux-brochure.pdf"');
                header('Expires: 0');
