...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO electric_brochure_form_submissions (first_name, last_name, email, phone, experience, country, inquiry_for) VALUES (:first_name, :last_name, :email, :phone, :experience, :country, :inquiry_for)";
        ...
        try {
            ...
            $mailer->isHTML(true);
            $mailer->Subject = 'Electric Vehicle Brochure Form Submission';

            $emailBody = "
                <html>
                <head>
                    <title>Electric Vehicle Brochure Form Submission</title>
                </head>
                <body>
                    <h1>Electric Vehicle Brochure Form Submission</h1>
            ...
            $brochureFile = 'electric-vehicle-designer-brochure.pdf'; // Replace with the actual path to your brochure file
            if (file_exists($brochureFile)) {
                header('Content-Description: File Transfer');
                header('Content-Type: application/octet-stream');
                header('Content-Disposition: attachment; filename="electric-vehicle-designer-brochure.pdf"');
                header('Expires: 0');
