...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO english_brochure_form_submissions (first_name, last_name, email, phone, experience, country, inquiry_for) VALUES (:first_name, :last_name, :email, :phone, :experience, :country, :inquiry_for)";
        ...
        try {
            ...
            $mailer->isHTML(true);
            $mailer->Subject = 'Spoken English Brochure Form Submission';

            $emailBody = "
                <html>
                <head>
                    <title>Spoken English Brochure Form Submission</title>
                </head>
                <body>
                    <h1>Spoken English Brochure Form Submission</h1>
            ...
            $brochureFile = 'spoken-english-poster.pdf'; // Replace with the actual path to your brochure file
            if (file_exists($brochureFile)) {
                header('Content-Description: File Transfer');
                header('Content-Type: application/octet-stream');
                header('Content-Disposition: attachment; filename="spoken-english-poster.pdf"');
                header('Expires: 0');
